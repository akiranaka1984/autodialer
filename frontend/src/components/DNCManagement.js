// frontend/src/components/DNCManagement.js
...
const DNCManagement = () => {
  ...
  const fetchDNCList = async () => {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        setTimeout(() => {
          setDncList([
            { id: 1, phone: '09012345678', reason: 'ユーザーリクエスト', created_at: '2025-05-01T10:00:00Z' },
            { id: 2, phone: '09023456789', reason: '不在着信多数', created_at: '2025-05-02T14:30:00Z' },
            { id: 3, phone: '09034567890', reason: 'クレーム', created_at: '2025-05-03T09:15:00Z' }
          ]);
  ...
  const handleImportDNC = async (e) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        setTimeout(() => {
          setSuccessMessage('3件をインポートしました');
          fetchDNCList();
        }, 1000);
        return;
      }
  ...
  const handleExportDNC = async () => {
    try {
      ...
      // 開発環境のモック処理
      if (process.env.NODE_ENV === 'development') {
        const csvContent = `電話番号,理由,登録日時
09012345678,ユーザーリクエスト,2025-05-01 10:00:00
09023456789,不在着信多数,2025-05-02 14:30:00
09034567890,クレーム,2025-05-03 09:15:00`;
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold flex items-center">
          <Ban className="h-8 w-8 mr-2 text-red-500" />
          着信拒否リスト（DNC）管理
        </h1>
