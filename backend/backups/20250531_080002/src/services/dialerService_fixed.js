// backend/src/services/dialerService.js - バグ修正版
...
class DialerService {
  ...
  async initialize() {
    ...
    try {
      ...
      // 🔥 バグ修正: 発信対象の連絡先があるキャンペーンのみ処理
      let validCampaigns = 0;
      for (const campaign of activeCampaigns) {
        try {
          const contactCountResult = await this.queryWithTimeout(
            'SELECT COUNT(*) as count FROM contacts WHERE campaign_id = ? AND status = "pending" LIMIT 1',
            [campaign.id], 3000
          );
          
          // 🔥 バグ修正: 安全なデータアクセス
          const contactCount = (contactCountResult && contactCountResult.length > 0 && contactCountResult[0]) 
            ? contactCountResult[0].count 
            : 0;
          ...
          if (contactCount > 0) {
            ...
            validCampaigns++;
            logger.info(`✅ キャンペーン追加: ${campaign.name} (連絡先: ${contactCount}件)`);
          } else {
            logger.info(`⏭️ キャンペーンスキップ: ${campaign.name} (連絡先なし)`);
          }
        } catch (campaignError) {
          logger.warn(`キャンペーン${campaign.id}の処理エラー:`, campaignError.message);
        }
      }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (Array.isArray(result) && result.length >= 1) {
          resolve(result[0]); // 最初の要素（行データ配列）を返す
        } else {
          resolve(result);
        }
      } catch (error) {
  ...
  async checkCampaignCompletion(campaignId) {
    try {
      const pendingResult = await this.queryWithTimeout(
        'SELECT COUNT(*) as count FROM contacts WHERE campaign_id = ? AND status = "pending"',
        [campaignId], 3000
      );
      
      const pendingCount = pendingResult && pendingResult.length > 0 && pendingResult[0] 
