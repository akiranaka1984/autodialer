// diagnose-dialer-fixed.js - 現在のdialerServiceに対応した診断
...
async function diagnoseDalerService() {
  ...
  try {
    // 1. 基本的なプロパティ確認
    console.log('📊 DialerService基本状態:');
    console.log(`  🔧 有効: ${dialerService.enabled !== false ? '✅' : '❌'}`);
    console.log(`  🚀 自動発信ID: ${dialerService.dialerIntervalId ? '✅ 動作中' : '❌ 停止'}`);
    console.log(`  👁️ キャンペーン監視ID: ${dialerService.campaignWatcherIntervalId ? '✅ 動作中' : '❌ 停止'}`);
    console.log(`  ⚡ 処理中: ${dialerService.isProcessing ? '⏳' : '待機中'}`);
    console.log(`  📞 アクティブキャンペーン: ${dialerService.activeCampaigns ? dialerService.activeCampaigns.size : '不明'}件`);
    console.log(`  📞 アクティブコール: ${dialerService.activeCalls ? dialerService.activeCalls.size : '不明'}件`);
    
    // 2. 利用可能なメソッド確認
    console.log('\n🔧 利用可能なメソッド:');
    const methods = Object.getOwnPropertyNames(Object.getPrototypeOf(dialerService))
      .filter(name => typeof dialerService[name] === 'function' && name !== 'constructor');
    console.log(`  発見されたメソッド: ${methods.join(', ')}`);
    
    // 3. アクティブキャンペーンの詳細（Mapの場合）
    if (dialerService.activeCampaigns && dialerService.activeCampaigns.size > 0) {
      ...
      for (const [id, campaign] of dialerService.activeCampaigns.entries()) {
        console.log(`  - ID ${id}: ${campaign.name || 'Unknown'}`);
        console.log(`    通話中: ${campaign.activeCalls || 0}/${campaign.maxConcurrentCalls || 1}`);
    ...
    dbCampaigns.forEach(campaign => {
      const inMemory = dialerService.activeCampaigns ? 
        dialerService.activeCampaigns.has(campaign.id) : false;
    ...
    try {
      ...
      console.log(`  ❌ SIPサービス取得エラー: ${sipError.message}`);
    }
    ...
    try {
      ...
      issues.push('SIPサービスアクセスエラー');
      recommendations.push('SIPサービス初期化を確認');
    }
    
    const dbCampaignIds = new Set(dbCampaigns.filter(c => c.pending_count > 0).map(c => c.id));
    const memoryCampaignIds = dialerService.activeCampaigns ? 
      new Set(Array.from(dialerService.activeCampaigns.keys())) : new Set();
    ...
    if (recentCalls.length === 0 && memoryCampaignIds.size > 0) {
...
if (require.main === module) {
  diagnoseDalerService().then(result => {
    ...
    if (result.needsRestart) {
      ...
      console.log('   node restart-dialer-fixed.js');
    } else {
