    ...
    try {
	  ...
	  stdio: 'inherit',
        ...
        const respondOnce = (success, error = null) => {
          ...
          if (success) {
            logger.info(`🎉 [成功] respondOnce(true)実行: ${callId} - 時刻: ${new Date().toISOString()}`);
        resolve(true);
          } else {
        ...
        pjsuaProcess.stdout?.on('data', (data) => {
          ...
          // 🎵 音声関連出力の詳細確認
          if (output.includes('play') || output.includes('audio') || output.includes('WAV') || 
              output.includes('file') || output.includes('stream')) {
            logger.info(`🎵 [音声] pjsua音声関連出力: ${callId} - ${output.substring(0, 100)}`);
          }
        ...
        setTimeout(() => {
          if (!hasResponded && pjsuaProcess.pid) {
            logger.info(`📞 [3秒タイマー] SIPプロセス正常開始: ${callId} - 時刻: ${new Date().toISOString()}`);
          logger.info(`🕐 [3秒タイマー] respondOnce(true)を実行します: ${callId}`);
